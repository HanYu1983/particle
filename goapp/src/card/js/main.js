...
Main.selectOps = function(ops) {
	try {
		...
		org_puremvc_haxe_patterns_facade_Facade.getInstance().sendNotification("setOpponents",per_vic_pureMVCref_tableGameModel_controller_SocketController.otherPlayerIds);
...
Main.loadDetail = function(game) {
	...
	if(Reflect.field(Main.cardSuitsDetails,game) == null) {
		CallJs.cardinfoloader_load(game,"ch",Main.onLoadGameCallback(game));
	}
};
...
Main.createItem = function(ary_data) {
	org_puremvc_haxe_patterns_facade_Facade.getInstance().sendNotification("do_create_item",per_vic_pureMVCref_tableGameModel_Tool.createItemFromData(ary_data));
};
...
Main.prototype = {
	onHtmlClick: function(type,params) {
		...
		case "onBtnStartServer":
			...
			org_puremvc_haxe_patterns_facade_Facade.getInstance().sendNotification("createPlayerSocket",per_vic_pureMVCref_tableGameModel_controller_SocketController.playerId);
			break;
		case "onBtnTableDeck":
		...
		case "onPokerClick":
			...
			var data1 = ary_poker.map(function(idstr1) {
				return { extra : [idstr1,"34","poker"], pos : [100,100], type : "card", width : 50, height : 75, back : false, lock : false, owner : per_vic_pureMVCref_tableGameModel_controller_SocketController.playerId};
			});
		...
		case "onTokenClick":
			...
			var data3 = ary_token.map(function(idstr3) {
				return { extra : [idstr3,"other"], pos : [100,100], type : "token", width : 50, height : 50, owner : per_vic_pureMVCref_tableGameModel_controller_SocketController.playerId};
			});
...
var mediator_UI = function(mediatorName,viewComponent) {
	...
	this.mc_messagePanel.find("#txt_messageInput").textbox({ onChange : function(nv,ov) {
		if(nv != "") {
			...
			_gthis.addSingleMessage(per_vic_pureMVCref_tableGameModel_controller_SocketController.playerId,nv);
			_gthis.sendNotification("sendMessage",{ type : "chat", msg : { id : per_vic_pureMVCref_tableGameModel_controller_SocketController.playerId, msg : nv}});
		}
	}});
	this.mc_messagePanel.find("#mc_quickButton > a").linkbutton({ onClick : function() {
		...
		_gthis.addSingleMessage(per_vic_pureMVCref_tableGameModel_controller_SocketController.playerId,msg);
		_gthis.sendNotification("sendMessage",{ type : "chat", msg : { id : per_vic_pureMVCref_tableGameModel_controller_SocketController.playerId, msg : msg}});
	}});
...
mediator_UI.prototype = $extend(org_puremvc_haxe_patterns_mediator_Mediator.prototype,{
	listNotificationInterests: function() {
		return ["on_select_cards","on_dice","on_been_invite","on_socket_error","on_socket_success","on_getSuit_success","on_receiveOps","on_searchComplete","on_heartbeat_event","on_receiveMessage","on_createDeck_click","on_save_click","on_load_click","do_show_recevie"];
	}
	,handleNotification: function(notification) {
		var _gthis = this;
	...
	,addSingleMessage: function(id,msg) {
		...
		if(id == per_vic_pureMVCref_tableGameModel_controller_SocketController.playerId) {
...
per_vic_pureMVCref_tableGameModel_controller_MainController.prototype = $extend(org_puremvc_haxe_patterns_mediator_Mediator.prototype,{
	...
	,updateView: function(ary_item,zs,apply) {
		...
		Lambda.foreach(ary_item,function(itemModel7) {
			...
			var item7 = js_Boot.__cast(m , per_vic_pureMVCref_tableGameModel_view_IItem);
	...
	,onBodyKeyUp: function(e) {
		...
		case 84:
			...
			this.sendNotification("on_dice",{ playerId : per_vic_pureMVCref_tableGameModel_controller_SocketController.playerId, dice : dice});
			this.sendNotification("sendMessage",{ type : "dice", msg : { playerId : per_vic_pureMVCref_tableGameModel_controller_SocketController.playerId, dice : dice}});
			break;
		case 86:
			this.setModelViewer();
			this.sendNotification("on_select_cards",{ ary_select : this.ary_select});
	...
	,setModelLock: function() {
		...
		while(_g1 < _g) {
			...
			if(itemModel.owner == "" || itemModel.owner == per_vic_pureMVCref_tableGameModel_controller_SocketController.playerId) {
	...
	,setModelOwner: function() {
		...
		while(_g1 < _g) {
			var i = _g1++;
			var itemModel = this.ary_select[i];
			var item = js_Boot.__cast(this.facade.retrieveMediator(itemModel.id) , per_vic_pureMVCref_tableGameModel_view_IItem);
			if(itemModel.owner == per_vic_pureMVCref_tableGameModel_controller_SocketController.playerId) {
				itemModel.owner = "";
			} else if(itemModel.owner == "") {
				itemModel.owner = per_vic_pureMVCref_tableGameModel_controller_SocketController.playerId;
			}
		}
	}
	,setModelViewer: function() {
		...
		while(_g1 < _g) {
			...
			var itemModel = this.ary_select[i];
			var item = js_Boot.__cast(this.facade.retrieveMediator(itemModel.id) , per_vic_pureMVCref_tableGameModel_view_IItem);
			if(itemModel.viewer == per_vic_pureMVCref_tableGameModel_controller_SocketController.playerId) {
				itemModel.viewer = "";
			} else {
...
per_vic_pureMVCref_tableGameModel_controller_SocketController.prototype = $extend(org_puremvc_haxe_patterns_mediator_Mediator.prototype,{
		...
		while(_g1 < _g) {
			...
			var fn = (function(_i) {
				return function(conn) {
					per_vic_pureMVCref_tableGameModel_controller_SocketController.otherPlayerIdsForCheck[_i[0]] = conn;
					per_vic_pureMVCref_tableGameModel_controller_SocketController.isConntect = Lambda.fold(per_vic_pureMVCref_tableGameModel_controller_SocketController.otherPlayerIdsForCheck,(function() {
					...
					if(per_vic_pureMVCref_tableGameModel_controller_SocketController.isConntect) {
						_gthis.sendNotification("on_searchComplete");
					}
					_gthis.sendNotification("on_heartbeat_event",{ conn : per_vic_pureMVCref_tableGameModel_controller_SocketController.isConntect});
				};
			})([i]);
	...
	,messageSocket: function(type,msg) {
		...
		if(!per_vic_pureMVCref_tableGameModel_controller_SocketController.isCanSendMessage) {
			return;
		}
		...
		Lambda.foreach(this.ary_ops,function(op) {
			if(op != per_vic_pureMVCref_tableGameModel_controller_SocketController.playerId) {
...
per_vic_pureMVCref_tableGameModel_view_BasicItem.prototype = $extend(org_puremvc_haxe_patterns_mediator_Mediator.prototype,{
	...
	,setOwner: function(o) {
		this._owner = o;
		if(this._owner == per_vic_pureMVCref_tableGameModel_controller_SocketController.playerId) {
