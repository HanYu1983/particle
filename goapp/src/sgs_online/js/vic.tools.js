...
(function( module ){
	...
	function createSceneByDae( daeScene ){
		...
		for ( var i = 0; i < daeScene.children.length; ++i ) {
			...
			switch( obj.name ){
				case 'Camera':
					createObj = new THREE.PerspectiveCamera();
					createObj.position.x = obj.position.x;
					createObj.position.y = obj.position.y;
					createObj.position.z = obj.position.z;
					camera = createObj;
					break;
	...
	*/
	...
	module.createSceneByDae = createSceneByDae;
