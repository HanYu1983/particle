package view 
{
	 ...
	 * @author vic
	 */
	public class UIurl extends EventDispatcher 
	{
		private var _ary_cardView:Array = [];
		private var _ary_imgPool:Object = { };
		
		public function UIurl( target:flash.events.IEventDispatcher=null ) 
		{
			super(target);
		...
		function drawOneCard( template:*, packageName:String, card:*, cb:Function ):void {
			...
			switch( packageName ) {
				case 'fighter':
					...
					var wtype = card[4];
					var atk = card[5];
					var buff:String = card[6];
					var ability:String = card[7];
					var skill:String = card[8];
					
					cardView['txt_weapon_name'].text = name;
					cardView['txt_weapon_level'].text = level;
					cardView['txt_weapon_weight'].text = weight;
					cardView['txt_weapon_power'].text = atk;
					cardView['txt_weapon_type'].text = wtype;
					cardView['txt_buff'].text = buff;
					cardView['txt_ability'].text = ability;
					cardView['txt_skill'].text = skill;
					
					cardView['mc_color'].gotoAndStop( 4 );
					
					if ( buff.length > 3 ) {
						cardView['mc_color'].gotoAndStop( 1 );
					}
					if ( ability.length > 3 ) {
						cardView['mc_color'].gotoAndStop( 4 );
					}
					if ( skill.length > 3 ) {
						cardView['mc_color'].gotoAndStop( 2 );
					}
				...
				case 'army':
					...
					*/
					...
					if ( url != '' ) {
						...
						ul.contentLoaderInfo.addEventListener(Event.COMPLETE, function( e:Event ):void {
							var img:DisplayObject = ul.contentLoaderInfo.content;
							//img.width = 744;
							//img.height = 1039;
							img.width = 744;
							img.height = 644;
						...
						ul.contentLoaderInfo.addEventListener(IOErrorEvent.IO_ERROR, function( e:IOErrorEvent ):void {
							finishOne();
						});
					}else {
						finishOne();
		...
		function appendCard( e:VicEvent ):void {
			...
			function doDrawOne():void {
				trace( 'doDrawOne' );
				if ( cardData.length > 0 ) {
					drawOneCard( template, packageName, cardData.shift(), doDrawOne );
				}else {
					startOutputImage();
				}
			}
			
			function startOutputImage():void {
				trace( 'startOutputImage' );
			...
			cardData.forEach( function( card ) {
				drawOneCard( template, packageName, card );
			});
			*/
			/*
			var t:Timer = new Timer( 100 );
